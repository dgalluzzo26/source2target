...
// Select a transform combo for an option - just track selection, don't regenerate SQL
// The SQL was already generated by the template with JOIN etc - we just highlight the selection
function selectTransformCombo(optionId: string, transforms: string) {
  selectedTransformCombo.value[optionId] = transforms
  
  // Find the option and update its transformations display
  const option = aiStore.unifiedOptions.find(o => o.id === optionId)
  if (option) {
    // Just update the transformations string for display
    // DON'T regenerate SQL - that would lose any JOIN configuration
    option.transformations = transforms
    console.log('[AISuggestions] Selected transform combo:', transforms, '(not regenerating SQL to preserve JOIN)')
  }
}
